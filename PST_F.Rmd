...
AnalyticData <- AnalyticData %>%
  mutate(age_in_days = as.numeric(difftime(calc_dod, calc_dob, units = "days")),
         ...
         infant_subgroup = case_when(
           age_group == "Infant (28 days to less than 12 months)" & age_in_days >= 28 & age_in_days < 182 ~ "28 days to <6 months",
           age_group == "Infant (28 days to less than 12 months)" & age_in_days >= 182 & age_in_days < 365 ~ "6 months to <12 months",
           TRUE ~ NA_character_
         ),
         
          # Create neonate subgroup
         Neonatal_Age_Group = case_when(
           age_group == "Stillbirth" ~ "Stillbirth",
           age_group == "Death in the first 24 hours" ~ "<24 hr",
           age_group == "Early Neonate (1 to 6 days)" & age_in_days >= 1 & age_in_days < 3 ~ "1-2 days",
           age_group == "Early Neonate (1 to 6 days)" & age_in_days >= 3 & age_in_days <= 6 ~ "3-6 days",
           age_group == "Late Neonate (7 to 27 days)" ~ "7 to 27 days",
           TRUE ~ NA_character_
  ))
...
TacData_K <- TacData_K %>%
  semi_join(AnalyticData_K1, by = "Champsid") %>%
  select(-any_of("month_old")) %>%  
...
```{r KP, include=FALSE, warning = FALSE}
...
AnalyticData_K <- AnalyticData_K %>%
  mutate(
    Kp_Causal_Chain = ifelse(
     ...
     Neonatal_Age_Group = factor(Neonatal_Age_Group,
                                levels = c("Stillbirth", "<24 hr", "1-2 days", "3-6 days", "7 to 27 days"))
  )
...
summary_table %>%
  ...
  tab_header(
    title = md("**Summary of *K. pneumoniae* in Neonatal Infectious Deaths**")
  ) %>%
  tab_style(
    style = cell_fill(color = "purple"),
    locations = cells_column_labels()
...
```{r EC, include=FALSE, warning = FALSE}
...
# Identify E. coli in causal chain
AnalyticData_E <- AnalyticData_E %>%
  mutate(
    Ec_Causal_Chain = ifelse(
      apply(select(., all_of(vars_eti)), 1, function(row) any(grepl("Escherichia coli|Escherichia coli/Shigella spp.", row, ignore.case = TRUE))),
      1, 0
    ),
    Neonatal_Age_Group = factor(Neonatal_Age_Group,
                                levels = c("Stillbirth", "<24 hr", "1-2 days", "3-6 days", "7 to 27 days"))
...
summary_table %>%
  ...
  cols_label(
    Neonatal_Age_Group = "Neonatal Age Group (n=total MITS DeCoDed)",
    infectious_cases = "Infectious Cause in Chain",
    ec_cases = md("Summary of *E. coli* in Causal Chain (% of infectious)")
  ) %>%
  tab_header(
...
```{r EC2, include=FALSE, warning = FALSE}

#### Neonatal Deaths for Escherichia coli/Shigella spp.####
...
AnalyticData_E <- AnalyticData_E %>%
  mutate(
    Ecs_Causal_Chain = ifelse(
      apply(select(., all_of(vars_eti)), 1, function(row) any(grepl("Escherichia coli/Shigella spp.", row, ignore.case = TRUE))),
      1, 0
    ),
    Neonatal_Age_Group = factor(Neonatal_Age_Group,
                                levels = c("Stillbirth", "<24 hr", "1-2 days", "3-6 days", "7 to 27 days"))
...
summary_table <- AnalyticData_E %>%
  ...
  summarise(
    total_deaths = n(),
    infectious_cases = sum(Infectious_Cause, na.rm = TRUE), 
    ecs_cases = sum(Ecs_Causal_Chain * Infectious_Cause, na.rm = TRUE) 
  ) %>%
  mutate(
    infectious_percent = ifelse(total_deaths > 0, round((infectious_cases / total_deaths) * 100), 0),
    ecs_percent = ifelse(infectious_cases > 0, round((ecs_cases / infectious_cases) * 100), 0)
  ) %>%
  mutate(
    total_deaths = paste0("n=", total_deaths),
    infectious_cases = paste0(infectious_cases, " (", infectious_percent, "%)"),
    ecs_cases = ifelse(!is.na(ecs_percent), paste0(ecs_cases, " (", ecs_percent, "%)"), paste0(ecs_cases, " (0%)"))
  ) %>%
  filter(!is.na(Neonatal_Age_Group))
...
```{r ECS, echo=FALSE}
summary_table %>%
  select(Neonatal_Age_Group, total_deaths, infectious_cases, ecs_cases) %>%
  gt() %>%
  cols_label(
    Neonatal_Age_Group = "Neonatal Age Group (n=total MITS DeCoDed)",
    infectious_cases = "Infectious Cause in Chain",
    ecs_cases = md("Summary of *E. coli/Shigella spp.* in Causal Chain (% of infectious)")
  ) %>%
  tab_header(
    title = md("**Summary of *E. coli/Shigella spp.* in Neonatal Infectious Deaths**")
  ) %>%
  tab_style(
    style = cell_fill(color = "darkseagreen"),
    locations = cells_column_labels()
...
acib_cases_by_age <- acib_cases_ch %>%
  group_by(age_group) %>%
...
age_group_summary <- total_deaths_by_age %>%
  ...
  mutate(
    ...
    Percentage_Acib = (Acib_Cases_ch / Total_Deaths) * 100
  )
...
age_group_summary <- age_group_summary %>%
  mutate(
    Acib_Cases_Combined = paste0(
      Acib_Cases_ch, " (", round(Percentage_Acib, 2), "%)"
    )
  ) %>%
  select(
    age_group,
...
TacData_A <- TacData_A %>%
  semi_join(AnalyticData_A1, by = "Champsid") %>%
  select(-any_of("month_old")) %>%
 
  left_join(select(AnalyticData_A1, Champsid, month_old), by = "Champsid") %>%
  filter(!is.na(month_old)) %>%
  mutate(
    A_baumannii_detected = ifelse(
      bld_ABAU_1 == "Positive" | CSF_ABAU_1 == "Positive" | lung_ABAU_1 == "Positive", 1, 0
    )
  )
...
AnalyticData_SP <- AnalyticData %>%
  mutate(age_group = recode(age_group, !!!age_group_mapping))

sp_cases_ch <- AnalyticData_SP %>%
  filter(apply(AnalyticData_SP[vars_eti], 1, function(row)
    any(grepl("Streptococcus pneumoniae", row, ignore.case = TRUE))
  ))

total_deaths_by_age <- AnalyticData_SP %>%
  group_by(age_group) %>%
  summarise(Total_Deaths = n(), .groups = "drop")

sp_cases_by_age <- sp_cases_ch %>%
  group_by(age_group) %>%
  summarise(SP_Cases_ch = n(), .groups = "drop")

TacData_SP <- TacData %>% 
  rename(Champsid = ChampsID)

Tac_filtered <- TacData_SP %>%
  semi_join(AnalyticData_SP, by = "Champsid") %>%                                      # keep only matched IDs
  left_join(select(AnalyticData_SP, Champsid, age_group), by = "Champsid") %>%
  mutate(
    SP_detected = ifelse(
      bld_STPN_2  == "Positive" |
      CSF_STPN_2  == "Positive" |
      lung_STPN_2 == "Positive", 1, 0
    )
  )

sp_detected <- Tac_filtered %>%
  group_by(age_group) %>%
  summarise(SP_Detected_Deaths = sum(SP_detected, na.rm = TRUE), .groups = "drop")

age_group_summary <- total_deaths_by_age %>%
  left_join(sp_cases_by_age, by = "age_group") %>%
  left_join(sp_detected,   by = "age_group") %>%
  mutate(
    SP_Cases_ch        = replace_na(SP_Cases_ch, 0),
    SP_Detected_Deaths = replace_na(SP_Detected_Deaths, 0),
    Percentage_SP      = (SP_Cases_ch / Total_Deaths) * 100
  )
...
age_group_display <- age_group_summary %>%
  mutate(
    SP_Cases_Combined = paste0(SP_Cases_ch, " (", round(Percentage_SP, 2), "%)")
  ) %>%
  select(
    age_group,
    Total_Deaths,
...
total_row <- tibble(
  age_group           = "Total",
  Total_Deaths        = sum(age_group_summary$Total_Deaths, na.rm = TRUE),
  SP_Detected_Deaths  = sum(age_group_summary$SP_Detected_Deaths, na.rm = TRUE),
  SP_Cases_Combined   = {
    total_cases <- sum(age_group_summary$SP_Cases_ch, na.rm = TRUE)
    total_n     <- sum(age_group_summary$Total_Deaths, na.rm = TRUE)
    pct         <- ifelse(total_n > 0, round(100 * total_cases / total_n, 2), NA_real_)
    paste0(total_cases, " (", pct, "%)")
  }
)

final_summary <- bind_rows(age_group_display, total_row)
```
...
final_summary %>%
  ...
  cols_label(
    age_group          = "Age Group",
    Total_Deaths       = "Total Deaths",
    SP_Detected_Deaths = md("*S. pneumoniae* Detected Deaths"),
    SP_Cases_Combined  = md("*S. pneumoniae* Cases (Causal Chain) %")
  ) %>%
  tab_style(
  ...
  tab_style(
    style = cell_fill(color = "lightsteelblue"),
    locations = cells_body(rows = nrow(final_summary))  # highlight total row
  ) %>%
  tab_options(
    table.width     = pct(100),
    table.font.size = px(14)
...
location_data <- tibble(
  Location = locations,
  Total_People = sapply(locations, function(loc) sum(AnalyticData_SP$site_name == loc, na.rm = TRUE))
)
...
age_groups <- unique(AnalyticData_SP$age_group)

# Total people per age group
age_group_data <- tibble(
  Age_Group = age_groups,
  Total_People = sapply(age_groups, function(ag) sum(AnalyticData_SP$age_group == ag, na.rm = TRUE))
)


vars_eti_valid <- vars_eti[vars_eti %in% colnames(AnalyticData_SP)]
...
age_group_totals <- AnalyticData_SP %>%
  group_by(age_group) %>%
  summarise(total_people_age_group = n(), .groups = "drop")
...
```{r GBS,  include=FALSE, warning = FALSE}
...
# Identify S. pneumoniae in causal chain
AnalyticData_SP <- AnalyticData_SP %>%
  mutate(
...
summary_table %>%
  ...
  cols_label(
    Neonatal_Age_Group = "Neonatal Age Group (n=total MITS DeCoDed)",
    infectious_cases = "Infectious Cause in Chain",
    spn_cases = md("Summary of *S. pneumoniae* in Causal Chain (% of infectious)")
  ) %>%
  tab_header(
...
Table for % of neonatal deaths caused by Klebsiella or E.coli

% of neonatal deaths caused by E. coli or GBS 

% of neonatal deaths caused by Klebsiella or GBS

```{r chain,  include=FALSE, warning = FALSE}
AnalyticData_KE_GBS <- AnalyticData_K %>%
  mutate(
    Ecoli_GBS_Chain = ifelse(
      apply(select(., all_of(vars_eti)), 1, function(row)
        any(grepl("Escherichia coli|GBS|Streptococcus agalactiae", row, ignore.case = TRUE))
      ),
      1, 0
    ),
    Kleb_GBS_Chain = ifelse(
      apply(select(., all_of(vars_eti)), 1, function(row)
        any(grepl("Klebsiella pneumoniae|GBS|Streptococcus agalactiae", row, ignore.case = TRUE))
      ),
      1, 0
    ),
    Kp_Ecoli_Causal_Chain = ifelse(
      apply(select(., all_of(vars_eti)), 1, function(row)
        any(grepl("Klebsiella pneumoniae|Escherichia coli", row, ignore.case = TRUE))
      ),
      1, 0
    )
  )
...
```{r c,  include=FALSE, warning = FALSE}
summary_table_all <- AnalyticData_KE_GBS %>%
  group_by(Neonatal_Age_Group) %>%
  summarise(
    total_deaths = n(),
    infectious_cases = sum(Infectious_Cause, na.rm = TRUE),
    ecoli_gbs_cases = sum(Ecoli_GBS_Chain * Infectious_Cause, na.rm = TRUE),
    kleb_gbs_cases = sum(Kleb_GBS_Chain * Infectious_Cause, na.rm = TRUE),
    kp_ecoli_cases = sum(Kp_Ecoli_Causal_Chain * Infectious_Cause, na.rm = TRUE)
  ) %>%
  mutate(
    ecoli_gbs_percent = ifelse(infectious_cases > 0, round((ecoli_gbs_cases / infectious_cases) * 100), 0),
    kleb_gbs_percent = ifelse(infectious_cases > 0, round((kleb_gbs_cases / infectious_cases) * 100), 0),
    kp_ecoli_percent = ifelse(infectious_cases > 0, round((kp_ecoli_cases / infectious_cases) * 100), 0),
    
    total_deaths = paste0("n=", total_deaths),
    infectious_cases = paste0(infectious_cases, " (", round((infectious_cases / as.numeric(gsub("n=", "", total_deaths))) * 100), "%)"),
    
    ecoli_gbs_cases = paste0(ecoli_gbs_cases, " (", ecoli_gbs_percent, "%)"),
    kleb_gbs_cases = paste0(kleb_gbs_cases, " (", kleb_gbs_percent, "%)"),
    kp_ecoli_cases = paste0(kp_ecoli_cases, " (", kp_ecoli_percent, "%)")
  ) %>%
  filter(!is.na(Neonatal_Age_Group))
...
summary_table_all %>%
  ...
  cols_label(
    Neonatal_Age_Group = "Neonatal Age Group (n=total MITS DeCoDed)",
    infectious_cases = "Infectious Cause in Chain",
    ecoli_gbs_cases = md("*E. coli or GBS*<br>(% of infectious)"),
    kleb_gbs_cases = md("*Klebsiella or GBS*<br>(% of infectious)"),
    kp_ecoli_cases = md("*K. pneumoniae or E. coli*<br>(% of infectious)")
  ) %>%
  tab_header(
    title = md("**Neonatal Infectious Deaths: Pathogens in Causal Chain**")
  ) %>%
  tab_style(
    style = cell_fill(color = "purple"),
    locations = cells_column_labels()
...
```{r grah, include=FALSE, warning = FALSE}
...
# Only extract the year from DateOfDeathNotification
AnalyticData_K <- AnalyticData_K %>%
  mutate(
    death_date = suppressWarnings(ymd(DateOfDeathNotification)),
    Year = year(death_date)
  )


kleb_long <- AnalyticData_K %>%
  filter(
    kleb_flag,
    age_group %in% c("Stillbirth", "Death in the first 24 hours"),
    site_name %in% locations,
    !is.na(Year)
  ) %>%
  transmute(
    site_name,
    Status = factor(age_group, levels = c("Stillbirth", "Death in the first 24 hours")),
    Year = as.integer(Year)
  )


yearly_counts <- kleb_long %>%
  count(Year, site_name, Status, name = "Cases")

# Fill missing year-site-status combos with 0
year_range <- range(yearly_counts$Year, na.rm = TRUE)
yearly_counts <- yearly_counts %>%
  complete(
    Year = seq.int(year_range[1], year_range[2]),
    site_name = unique(kleb_long$site_name),
    Status = levels(kleb_long$Status),
    fill = list(Cases = 0)
  )
...
panel_counts <- yearly_counts %>%
  mutate(Panel = as.character(Status)) %>%
  select(Year, site_name, Cases, Panel) %>%
  bind_rows(combined_counts) %>%
  mutate(
    Panel = factor(Panel, levels = c("Death in the first 24 hours", "Stillbirth", "Combined"))
  )

min_year <- min(panel_counts$Year, na.rm = TRUE)
max_year <- max(panel_counts$Year, na.rm = TRUE)
y_max <- ceiling(max(panel_counts$Cases, na.rm = TRUE) / 5) * 5



```

```{r graph_c, echo=FALSE}
p3 <- ggplot(panel_counts, aes(x = Year, y = Cases, color = site_name, group = site_name)) +
  geom_line() +
  geom_point() +
  ...
  labs(
    title = "K. pneumoniae stillbirths, <24h deaths, and combined per site",
    x = "Year",
    y = "Number of cases",
    color = "Country / Site"
  ) +
...
```{r GA, include=FALSE, warning = FALSE}

# Prefer GA Maternal Wks; if NA, use GA Child Wks
AnalyticData_K <- AnalyticData_K %>%
  mutate(
    ga_wks = ifelse(is.na(`GA_maternal_wks`), `GA_child_wks`, `GA_maternal_wks`)
  )


# summary(AnalyticData_K$`GA Child Wks`)
# summary(AnalyticData_K$`GA Maternal Wks`)
# summary(AnalyticData_K$ga_wks)


AnalyticData_K <- AnalyticData_K %>%
  mutate(
    ga_trimester = dplyr::case_when(
      is.na(ga_wks)               ~ "Missing",
      ga_wks <= 12                ~ "Q1: ≤12 wks",
      ga_wks >= 13 & ga_wks <=28  ~ "Q2: 13–28 wks",
      ga_wks >= 29                ~ "Q3: ≥29 wks",
      TRUE                        ~ "Missing"
    ),
    ga_trimester = factor(
      ga_trimester,
      levels = c("Q1: ≤12 wks","Q2: 13–28 wks","Q3: ≥29 wks","Missing")
    )
  )


kleb_flag <- apply(search_matrix, 1, function(row)
  any(grepl("Klebsiella\\s*pneumoniae|\\bK\\.?\\s*pneumoniae\\b", row, ignore.case = TRUE)))

ecoli_flag <- apply(search_matrix, 1, function(row)
  any(grepl("Escherichia\\s*coli(\\/Shigella\\s*spp\\.)?|\\bE\\.?\\s*coli\\b", row, ignore.case = TRUE)))

gbs_flag <- apply(search_matrix, 1, function(row)
  any(grepl("Group\\s*B\\s*Streptococcus|Streptococcus\\s*agalactiae|\\bGBS\\b", row, ignore.case = TRUE)))


AnalyticData_K <- AnalyticData_K %>%
  mutate(
    kleb_flag = kleb_flag,
    ecoli_flag = ecoli_flag,
    gbs_flag = gbs_flag
  )
...
neonates <- AnalyticData_K %>%
  filter(age_group_clean %in% neonatal_levels)


pathogen_long <- neonates %>%
  select(ga_wks, ga_trimester, kleb_flag, ecoli_flag, gbs_flag) %>%
  pivot_longer(
    cols = c(kleb_flag, ecoli_flag, gbs_flag),
    names_to = "Pathogen",
    values_to = "flag"
  ) %>%
  filter(flag) %>%
  mutate(
    Pathogen = recode(
      Pathogen,
      kleb_flag  = "Klebsiella pneumoniae",
      ecoli_flag = "Escherichia coli",
      gbs_flag   = "Group B Streptococcus"
    )
  )


tri_tbl <- pathogen_long %>%
  group_by(ga_trimester, Pathogen) %>%
  ...
  mutate(
    Total = `Klebsiella pneumoniae` + `Escherichia coli` + `Group B Streptococcus`
  ) %>%
  arrange(ga_trimester)

```



```{r GA_graph, echo=FALSE}
gt_tri <- tri_tbl %>%
  rename(`Gestational Age (Trimester)` = ga_trimester) %>%
  gt() %>%
  cols_label(
    `Klebsiella pneumoniae` = md("*K. pneumoniae*"),
    `Escherichia coli` = md("*E. coli*"),
    `Group B Streptococcus` = md("*GBS*"),
    Total = "Total"
  ) %>%
  tab_header(
    title = md("**Neonatal Deaths by Gestational Age (Trimesters)**")
  ) %>%
  tab_style(
    style = cell_fill(color = "purple"),
    locations = cells_column_labels()
